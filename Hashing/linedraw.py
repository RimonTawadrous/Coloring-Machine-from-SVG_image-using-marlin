...
no_cv = False
hatch_size = 100
imageName = "cosmo&wanda.png"
export_path = "output/"+imageName+".svg"
...
def sketch(path):
    ...
    disp = ImageOps.invert(disp)

    img = cv2.imread(image_path)
    img = cv2.resize(img, (resolution,int(resolution*h/w)) )
    img = cv2.cvtColor( img , cv2.COLOR_BGR2RGB )

    final_img = np.bitwise_and(img , disp)
    final_img[final_img == 0] = 255
    #print(I2.shape , I1.shape)

    disp = Image.fromarray(final_img)
    disp.show()
    disp.save("png_output"+"/"+image_path[0:-4]+"_"+str(hatch_size)+"."+image_path[-3:])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--input',dest='input_path',
        default='tweety',action='store',nargs='?',type=str,
        help='Input path')
